...
// Copyright (c) 2017-2018 The Bzlcoin developers
...
const string strMessageMagic = "BZLCoin Signed Message:\n";
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (fTestNet){
        if (pindex->nHeight > BLOCK_START_MASTERNODE_PAYMENTS_TESTNET){ // testnet
        ...
        if (pindex->nHeight > BLOCK_START_MASTERNODE_PAYMENTS){ //Block 342k Mainnet
            MasternodePayments = true;
            if(fDebug) { printf("CheckBlock() : Masternode payments enabled\n"); }
        }else{
            MasternodePayments = false;
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Bzlcoin: ask for pending sync-checkpoint if any
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        strMiscWarning = strMessage;
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BZLCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "https://www.coindesk.com/survey-reveals-massive-support-for-blockchain-in-healthcare-systems/";
        CTransaction txNew;
        txNew.nTime = 1507502500;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xe57c8619ce867c55590c46ae93694799f8db4e6267f9ce69855361027501044e"));
        block.print();
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version")
    {
        ...
        // Disconnect nodes that are over block height 37k and have an old peer version
        if (nBestHeight >= 400000 && pfrom->nVersion < PROTOCOL_VERSION)
            oldVersion = true;
...
{
    //int64_t ret = blockValue * 1/5; //20%
	int64_t ret = static_cast<int64_t>(blockValue * 1/5); //20%
