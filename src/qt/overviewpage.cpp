...
const QString BaseURL = "https://bzlcoin.org/marketbzl.php?market=bzl.usd";
const QString BaseURL2 = "https://bzlcoin.org/marketbzl.php?market=market.btc";
double bzlcoinx;
double bzlbtcx;
...
if (what == BaseURL) // Bzlcoin Price
{

    // QNetworkReply is a QIODevice. So we read from it just like it was a file
    QString bzlcoin = finished->readAll();
    bzlcoinx = (bzlcoin.toDouble());
    bzlcoin = QString::number(bzlcoinx, 'f', 2);

	dollarg = bzlcoin;
}
if (what == BaseURL2) // Bzlcloin BTC Price
{

    // QNetworkReply is a QIODevice. So we read from it just like it was a file
    QString bzlbtc = finished->readAll();
    bzlbtcx = (bzlbtc.toDouble());
    bzlbtc = QString::number(bzlbtcx, 'f', 8);
...
void OverviewPage::setBalance(qint64 balance, qint64 lockedbalance, qint64 stake, qint64 unconfirmedBalance, qint64 immatureBalance, qint64 watchOnlyBalance, qint64 watchUnconfBalance, qint64 watchImmatureBalance)
{
	...
	//preço em Real
	QString totalbrl;
	double realarg2 = (realarg.toDouble() * totalBalance / 100000000);
