    ...
    address(addr)
{
    ...
    if (label.trimmed().isEmpty()) {
        setWindowTitle(QString("BZLCoin - Generate QR Code"));
    } else {
        setWindowTitle(QString("BZLCoin - Generate QR Code for %1").arg(label));
    }
...
{
    QString ret = QString("bzlcoin:%1").arg(address);
    int paramCount = 0;
