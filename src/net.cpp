...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "BZLCoin " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
static const char *strDNSSeed[][2] = {
	{"dnsseed.bzlcoin.org", "dnsseed.bzlcoin.org"},
	{"explorer.bzlcoin.org", "explorer.bzlcoin.org"},
	{"network.bzlcoin.org", "network.bzlcoin.org"},
	{"chainz.cryptoid.info", "chainz.cryptoid.info"},
	{"seed1.bzlcoin.org", "seed1.bzlcoin.org"}
};

void ThreadDNSAddressSeed(void* parg)
{
    // Make this thread recognisable as the DNS seeding thread
    RenameThread("bzlcoin-dnsseed");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. BZLCoin is probably already running."), addrBind.ToString().c_str());
        else
